...
import com.rzlearn.base.support.BASE64Encoder;
import com.rzlearn.base.support.coder.*;
...
public final class SecurityUtil {
	 ...
	 * 默认算法密钥
	 */
	private static final byte[] ENCRYPT_KEY = { -19, 89, 6, 11, -6, 0, 1, 9 };
	...
	public static final String decryptDes(String cryptData, byte[] key) {
		...
		try {
			// 把字符串解码为字节数组，并解密
			decryptedData = new String(DESCoder.decrypt(decryptBASE64(cryptData), key), "utf-8");
		} catch (Exception e) {
			throw new RuntimeException("解密错误，错误信息：", e);
		}
	...
	public static final String encryptDes(String data, byte[] key) {
		...
		try {
			// 加密，并把字节数组编码成字符串
			encryptedData = encryptBASE64(DESCoder.encrypt(data.getBytes("utf-8"), key));
		} catch (Exception e) {
			throw new RuntimeException("加密错误，错误信息：", e);
		}
	...
	public static final String decryptRSAPublic(String cryptData, String publicKey) {
		try {
			// 把字符串解码为字节数组，并解密
			return new String(RSACoder.decryptByPublicKey(decryptBASE64(cryptData), decryptBASE64(publicKey)), "utf-8");
		} catch (Exception e) {
			throw new RuntimeException("解密错误，错误信息：", e);
		}
	}
...
//		System.out.println(encryptDes("LTAIOjRSAYC29XOY"));
//		System.out.println(encryptDes("xz6GP4i2idInOP8HxR9k09p7ZjsEey"));
		System.out.println(decryptDes("oLRO/Bh7dJ6Tp0WCVbRexlAgejmp2uXI"));
		System.out.println(decryptDes("yjY6bOtsQ+Z0E0knImiIzys1Tb77z4v0B8APBQnr3uy="));
