...
public final class XmlUtil {
    ...
    public static final Map parseXml2Map(String pStrXml) {
        ...
        document = parseStrXml(pStrXml, strTitle);
        // 获取根节点
    ...
    public static Map document2Map(Document doc) {
        Map map = new HashMap(16);
        if (doc == null) {
            return map;
        }
        ...
        for (Iterator iterator = root.elementIterator(); iterator.hasNext(); ) {
            ...
            if (list.size() > 0) {
                map.put(e.getName(), document2Map(e));
            } else {
    ...
    public static Map document2Map(Element e) {
        Map map = new HashMap(16);
        List list = e.elements();
        if (list.size() > 0) {
...
//        Element firstEl = (Element) document.selectSingleNode("/" + pRootNodeName + "/" + pFirstNodeName);
    ...
    private static Document parseStrXml(String pStrXml, String strTitle) {
        Document document;
        try {
            if (pStrXml.indexOf(XML_START) < 0) {
    ...
    public static Map<String, Object> dom2Map(Document doc) {
        Map<String, Object> maproot = new HashMap<String, Object>(16);
        if (doc == null) {
            return maproot;
        }
        Element root = doc.getRootElement();
    ...
    public static void element2Map(Element e, Map<String, Object> map) {
        ...
        if (list.size() < 1 && StringUtils.isEmpty(e.getText())) {
            return;
        } else if (list.size() < 1 && StringUtils.isNotEmpty(e.getText())) {
            map.put("text", e.getText());
        }
        initMap(map, list);
    }

    private static void initMap(Map<String, Object> map, List<Element> list) {
